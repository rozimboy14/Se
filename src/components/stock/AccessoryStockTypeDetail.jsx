...
function AccessoryStockTypeDetail() {
...
const handleExportPDF = async () => {
  try {
    const response = await getAccessoryStockExportPdf("", type_name,warehouseId);
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        const response = await getAccessoryStock("", type_name,warehouseId);
  ...
  return (
    <Container maxWidth="full">
      <Box sx={{ width: "100%", height: "100%" }}>
        <Box sx={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "5px",marginBottom:"4px", marginTop:"25px" }}>
          <Box sx={{ flex: 1, textAlign: "center" }}>
           <h3 style={{ margin: "0px",fontSize:"14px" }}>{warehouse_name} <span style={{color:"#2795F5",fontSize:"15px"}}>{type_name}</span> <span>Остатки</span></h3>
          </Box>
...
export default AccessoryStockTypeDetail
