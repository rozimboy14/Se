...
function AccessoryStockDetailAll() {
  ...
  const handleExportPDF = async () => {
    try {
      const response = await getAccessoryStockExportPdf("", "", "");
      const url = window.URL.createObjectURL(new Blob([response.data], { type: 'application/pdf' }));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `${warehouse_name}-все аксессуарь`); // fayl nomi
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        const response = await getAccessoryStock("", "", warehouseId);
...
export default AccessoryStockDetailAll
