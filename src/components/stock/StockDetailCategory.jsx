...
import { getStock } from "../api/axios";
import { useParams, useSearchParams } from "react-router-dom";

function StockDetailCategory() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        const response = await getStock("", category_id, warehouseId);
              ...
              {stock?.map((item, index) => (
                <React.Fragment key={item.id}>
                  <TableRow sx={{ padding: "0 10px" }}  >
                    ...
                    <TableCell sx={{ textAlign: "center", fontSize: "13px", padding: "6px 10px" }}> {Number(item.total_quantity).toLocaleString("ru-RU")}</TableCell>
...
export default StockDetailCategory
