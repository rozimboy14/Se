...
import { deletePackagingCategory, getPackagingCategory, postPackagingCategory, patchPackagingCategory } from "../api/axios";
...
function PackagingCategory() {
  ...
  const [pCategoty, setPCategory] = useState([])
    ...
    const handleDelete = (id) => {
        const item = pCategoty.find((b) => b.id === id);
      ...
      useEffect(() => {
        const fetchBrand = async () => {
          ...
          try {
            const response = await getPackagingCategory();
      ...
      const handleAddCategory = async (item) => {
        ...
        try {
          const response = await postPackagingCategory(data);
          setPCategory(prev => [...prev, response.data]);
        } catch (err) {
      ...
      const handleEditSave = async (newData) => {
        ...
        try {
          const res = await patchPackagingCategory(editCategory.id, newData);
          setPCategory(prev =>
      ...
      const confirmDelete = async () => {
        ...
        try {
          await deletePackagingCategory(selectedCategory.id);
          setPCategory(prev => prev.filter(b => b.id !== selectedCategory.id));
        } catch (err) {
...
<Container maxWidth="full">
  ...
  <Box sx={{ flex: 1, textAlign: "center" }}>
    <h3 style={{margin:0,marginBottom:"6px"}}>Категория упаковки</h3>
  </Box>
