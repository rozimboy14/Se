...
const EditCategory = ({ open, onClose, onEdit, initialValue, initialNorm }) => {
  const [name, setName] = useState("");
  const [norm, setNorm] = useState("");

  useEffect(() => {
    setName(initialValue || "");
    setNorm(initialNorm?.toString() || "");
  }, [initialValue, initialNorm]);

  const handleSubmit = () => {
    if (name.trim() && norm.trim()) {
      onEdit({ name: name.trim(), norm: parseFloat(norm) });
      onClose();
    }
  };
