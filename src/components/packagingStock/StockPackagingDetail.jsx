...
import { getPackagingStock, getStock, getStockBrandExportPdf, getStockPackaBrandgeExportPdf } from "../api/axios";
import { useParams, useSearchParams } from "react-router-dom";

function StockPackagingDetail() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        const response = await getPackagingStock(brand_Id,"",warehouseId);
    ...
    fetchOrders()
  }, [brand_Id,warehouseId])
  console.log(stock);

  const handleExportPDF = async () => {
    try {
      const response = await getStockPackaBrandgeExportPdf(warehouseId,brand_Id);
      ...
      <Box sx={{ width: "100%", height: "100%" }}>
        ...
        <TableContainer component={Paper} sx={{ width: "100%", minWidth: "0",backgroundColor: isDark ? "#0e1c26" : "white", }}>
          <Table>
            <TableHead>
              <TableRow>
                <TableCell sx={{ width: 10, padding: "2px 7px", fontSize: "12px" }} />
                ...
                <TableCell sx={{ width: '25%', textAlign: "center", fontSize: "12px", padding: "5px 10px" }}>Модель</TableCell>
                ...
                <TableCell sx={{ width: '15%', textAlign: "center", fontSize: "12px", padding: "5px 10px" }}>высший сорт</TableCell>
                <TableCell sx={{ width: '15%', textAlign: "center", fontSize: "12px", padding: "5px 10px" }}>2-сорт</TableCell>
              ...
              {stock?.map((item, index) => (
                <React.Fragment key={item.id}>
                  <TableRow sx={{ padding: "0 10px" }}  >
                    ...
                    <TableCell sx={{ textAlign: "center", fontSize: "12px", padding: "2px 6px" }}> {Number(item.total_sort_1).toLocaleString("ru-RU")}</TableCell>
                    <TableCell sx={{ textAlign: "center", fontSize: "12px", padding: "2px 6px" }}> {Number(item.total_sort_2).toLocaleString("ru-RU")}</TableCell>
...
export default StockPackagingDetail
