...
import { getPackagingStock, getStock, getStockExportPdf, getStockPackageExportPdf } from "../api/axios";
import { useParams, useSearchParams } from "react-router-dom";

function StockPackagingDetailAll() {
  ...
  const handleExportPDF = async () => {
    try {
      const response = await getStockPackageExportPdf(warehouseId);
      const url = window.URL.createObjectURL(new Blob([response.data], { type: 'application/pdf' }));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `${warehouse_name}-Все остатки`);
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        const response = await getPackagingStock("", "", warehouseId);
      ...
      <Box sx={{ width: "100%", height: "100%" }}>
        <Box sx={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "5px" }}>
          <Box sx={{ flex: 1, textAlign: "center" }}>
            <h3 style={{ margin: "0px", fontSize: "14px" }}>{warehouse_name} <span>Все Остатки</span></h3>
          </Box>
              ...
              {stock?.map((item, index) => (
                <React.Fragment key={item.id}>
                  <TableRow sx={{ padding: "0 10px" }}  >
                    ...
                    <TableCell sx={{ fontSize: "12px", padding: "2px 5px" }}>{index + 1}</TableCell>
                    <TableCell sx={{ textAlign: "center", fontSize: "12px", padding: "2px 5px" }}>{item.order.brand_name}</TableCell>
                    <TableCell sx={{ textAlign: "center", fontSize: "12px", padding: "2px 5px" }}>{item.order.article_name}</TableCell>
                    <TableCell sx={{ textAlign: "center", fontSize: "12px", padding: "2px 5px" }}>{item.order.specification_name}</TableCell>
                    <TableCell sx={{ textAlign: "center", fontSize: "12px", padding: "2px 5px" }}>{item.order.comment}</TableCell>
                    <TableCell sx={{ textAlign: "center", fontSize: "12px", padding: "2px 5px" }}> {Number(item.sort_1).toLocaleString("ru-RU")}</TableCell>
                    <TableCell sx={{ textAlign: "center", fontSize: "12px", padding: "2px 5px" }}> {Number(item.sort_2).toLocaleString("ru-RU")}</TableCell>
                        ...
                        <Paper sx={{
                          ...
                          {item.stock_packaging_variant?.length > 0 && (
                            <>
                              ...
                              <Table size="small" >
                                <TableHead>
                                  <TableRow sx={{ backgroundColor: isDark ? "#0e1c26" : "white", }}>

                                    <TableCell sx={{ fontSize: "10px", padding: "0px 5px", textAlign: "center" }}>Вариант</TableCell>
                                    <TableCell sx={{ fontSize: "10px", padding: "0px 5px", textAlign: "center" }}>высший сорт</TableCell>
                                    <TableCell sx={{ fontSize: "10px", padding: "0px 5px", textAlign: "center" }}>2-сорт</TableCell>
                                ...
                                <TableBody >
                                  {item.stock_packaging_variant.map((variant, idx) => (
                                    <TableRow key={variant.id}>
                                      ...
                                      <TableCell sx={{ fontSize: "12px", textAlign: "center", }}>{variant.variant_name}</TableCell>
                                      <TableCell sx={{ fontSize: "12px", textAlign: "center" }}>{variant.sort_1}</TableCell>
                                      <TableCell sx={{ fontSize: "12px", textAlign: "center" }}>{variant.sort_2}</TableCell>
...
export default StockPackagingDetailAll
